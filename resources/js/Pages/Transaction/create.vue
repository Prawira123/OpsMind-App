...
const form = useForm({
    ...
    other_fee:         0,
    rekening_id:       '',
    items:             [],  // ← dikirim sekaligus saat submit
})
...
<template>
    <div class="max-w-full mx-auto">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Transaksi</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    Isi detail transaksi dan tambahkan item
                </p>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between pt-2 pb-8">
                ...
                <button type="submit"
                    ...
                    {{
                        form.processing
                            ? 'Menyimpan...'
                            : `Simpan Transaksi${form.items.length > 0 ? ` (${form.items.length} item)` : ''}`
                    }}
                </button>
            </div>
